...
function SetCarsCount(count) {
    ...
    preview.innerHTML = `You can choose one of ${count} cars which you posted here`;
}
...
function ConfigureUrl() {
    ...
    if(brand === "-- brand --") {
        var url = `http://127.0.0.1:8000/cars/user/${localStorage.getItem("user_id")}?price=${price}`
    } else if(brand !== "-- brand --" && model === "-- model --") {
        var url = `http://127.0.0.1:8000/cars/user/${localStorage.getItem("user_id")}?brand=${brand}&price=${price}`
    } else if(brand !== "-- brand --" && model !== "-- model --" && 
                category === "-- type --" && fuel === "-- fuel --" &&
                color === "-- color --") {
        var url = `http://127.0.0.1:8000/cars/user/${localStorage.getItem("user_id")}?brand=${brand}&model=${model}&price=${price}`
    } else if(brand !== "-- brand --" && model !== "-- model --" && 
                category !== "-- type --" && fuel !== "-- fuel --" &&
                color !== "-- color --"){
        var url = `http://127.0.0.1:8000/cars/user/${localStorage.getItem("user_id")}?brand=${brand}&model=${model}&category=${category}&fuel=${fuel}&color=${color}&price=${price}`;
    }
...
async function GetCars(url) {
    ...
    for (let item of result_cars){
        ...
        if (localStorage.getItem("user_id") != item.owner_id) { continue; }
        ...
        updateButton.setAttribute("id", `update-button-${item.id}`);
...
await GetCars(`http://127.0.0.1:8000/cars/user/${localStorage.getItem("user_id")}`);


async function DeleteCar(carDeleteId) {
...
for (let updateButton of updateButtons) {
    updateButton.addEventListener('click', function(event) {
        ...
        var car_id = updateButton.id.substring(14);
